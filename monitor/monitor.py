...
def check_ssl(url):
    ...
    try:
        ...
        return {"expires_at": expires.isoformat().replace("+00:00", "Z"), "days_remaining": remaining.days}
...
def fetch_github_issues(repo):
    ...
    headers = {"Accept": "application/vnd.github+json", "User-Agent": "Gabriel-Ops-Monitor/1.0"}
...
def check_site(site):
    ...
    try:
        ...
        status = "offline"
        error = str(exc)
        issues.append({"severity": "critical", "title": "Site indisponível", "detail": str(exc)})
    except Exception as exc:
        latency_ms = round((time.perf_counter() - started) * 1000)
        status = "offline"
        error = str(exc)
        issues.append({"severity": "critical", "title": "Falha inesperada na checagem", "detail": str(exc)})

    ssl_info = check_ssl(site["url"])
    if ssl_info:
        if ssl_info.get("error"):
            issues.append({"severity": "warning", "title": "Falha ao validar SSL", "detail": ssl_info["error"]})
